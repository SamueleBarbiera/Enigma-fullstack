...
export async function getStaticPaths({ locales }: GetStaticPathsContext) {
    ...
    const paths = tags?.data?.flatMap((tag: Tag) => locales?.map((locale) => ({ params: { tags: tag.slug }, locale })))
...
export const getStaticProps: GetStaticProps = async ({ locale, params }) => {
    ...
    try {
        ...
        await Promise.all([
            ...
            await queryClient.prefetchInfiniteQuery([API_ENDPOINTS.PRODUCTS, { tags }], fetchInfiniteProducts),
        ])
