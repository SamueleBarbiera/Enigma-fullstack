...
export async function getStaticPaths({ locales }: GetStaticPathsContext) {
    ...
    const paths = data?.flatMap((shop: any) => locales?.map((locale) => ({ params: { slug: shop.slug }, locale })))
...
export const getStaticProps: GetStaticProps = async ({ params, locale }) => {
    ...
    try {
        ...
        await queryClient.prefetchInfiniteQuery([API_ENDPOINTS.PRODUCTS, { shop_id: shop?.id }], fetchInfiniteProducts)
        return {
            props: {
                data: { shop },
