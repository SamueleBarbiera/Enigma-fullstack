import { QueryParamsType, Order, OrdersQueryOptionsType } from '@framework/types'
...
const fetchOrders = async ({ queryKey }: QueryParamsType): Promise<{ orders: PaginatedOrder }> => {
    const [_key, params]: any = queryKey

    const { page, limit = 15, orderBy = 'updated_at', sortedBy = 'DESC' } = params as OrdersQueryOptionsType
...
const useOrdersQuery = (options: OrdersQueryOptionsType) => {
    return useQuery<{ orders: PaginatedOrder }, Error>([API_ENDPOINTS.ORDER, options], fetchOrders, {
...
export const useOrderQuery = ({ tracking_number }: { tracking_number: string }) => {
    return useQuery<{ order: Order }, Error>(['order', tracking_number], () => fetchOrder(tracking_number))
}
...
export const useCreateOrderMutation = () => {
    return useMutation((input: OrderCreateInputType) => OrderService.create(input))
}
