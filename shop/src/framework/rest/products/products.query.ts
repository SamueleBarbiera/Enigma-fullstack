import { QueryParamsType, ProductsQueryOptionsType, Product } from '@framework/types'
...
const fetchInfiniteProducts = async ({ queryKey, pageParam }: QueryParamsType): Promise<PaginatedProduct> => {
...
const useProductsInfiniteQuery = (
    ...
    options?: UseInfiniteQueryOptions<PaginatedProduct, Error, PaginatedProduct, PaginatedProduct, QueryKey>
) => {
    return useInfiniteQuery<PaginatedProduct, Error>([API_ENDPOINTS.PRODUCTS, params], fetchInfiniteProducts, {
        ...options,
...
export const useProductsQuery = (options: ProductsQueryOptionsType) => {
    return useQuery<{ data: Product[] }, Error>([API_ENDPOINTS.PRODUCTS, options], fetchProducts)
}
