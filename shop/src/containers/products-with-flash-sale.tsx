...
const ProductsWithFlashSale: React.FC<Props> = ({
    ...
    const { data: flashSellProduct, isLoading: flashSellProductLoading } = useProductsQuery({
        limit,
        tags: flashSaleSettings?.slug,
    })

    return (
        <div className={`grid grid-cols-1 gap-5 md:gap-14 xl:gap-7 xl:grid-cols-7 2xl:grid-cols-9 ${className}`}>
