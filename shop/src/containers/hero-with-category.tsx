...
import {Category, StaticBanner} from '@framework/types'
...
const HeroWithCategory = ({className = 'mb-12 md:mb-14 xl:mb-16', data, paginationPosition = 'center'}: Props) => {
    ...
    return (
        <div className={`grid grid-cols-1 2xl:grid-cols-5 gap-5 xl:gap-7 ${className}`}>
            {error ? (
                <Alert message={error?.message} />
            ) : width < 1500 ? (
                <div>
                    <Carousel breakpoints={categoryResponsive} buttonSize='small'>
                        {loading && !cat?.data?.length
                ...
                <div className='ltr:2xl:-mr-14 rtl:2xl:-ml-14 grid grid-cols-1 gap-3'>
                    {loading && !cat?.data?.length ? <CategoryListFeedLoader limit={8} /> : categories?.data?.slice(0, 8).map((category) => <CategoryListCard key={`category--key${category.id}`} category={category} />)}
                </div>
            )}
                ...
                >
                    {data?.map((banner: any) => (
                        <SwiperSlide key={`banner--key${banner.id}`}>
                            <BannerCard data={banner} href={`${ROUTES.COLLECTIONS}/${banner.slug}`} className='xl:h-full' />
                        </SwiperSlide>
                    ))}
                </Carousel>
            </div>
