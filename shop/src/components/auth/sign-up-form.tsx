        ...
        <div className='py-5 px-5 sm:px-8 bg-white mx-auto rounded-lg w-full sm:w-96 md:w-450px border border-gray-300'>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col justify-center' noValidate>
                <div className='flex flex-col space-y-4'>
                    <Input labelKey='forms:label-name-star' type='text' variant='solid' {...register('name')} errorKey={errors.name?.message} />
                    ...
                    <div className='relative'>
                        <Button type='submit' loading={isLoading} disabled={isLoading} className='h-11 md:h-12 w-full mt-2'>
            ...
            {/*</Button>*/}
            <Button type='submit' loading={false} disabled={isLoading} className='h-11 md:h-12 w-full mt-2.5 bg-google hover:bg-googleHover'>
                <ImGoogle2 className='text-sm sm:text-base ltr:mr-1.5 rtl:ml-1.5' />
                {t('common:text-login-with-google')}
            </Button>
            <div className='text-sm sm:text-base text-body text-center mt-5 mb-1'>
                {t('common:text-have-account')}{' '}
                <button type='button' className='text-sm sm:text-base text-heading underline font-bold hover:no-underline focus:no-underline focus:outline-none' onClick={handleSignIn}>
                    {t('common:text-login')}
                </button>
            </div>
        </div>
