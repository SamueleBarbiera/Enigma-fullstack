...
import { useOtpLoginMutation, useSendOtpCodeMutation } from '@framework/auth/auth.query'
...
const otpLoginFormSchema = yup.object().shape({
    ...
    code: yup.string().required('forms:error-code-required').min(6, 'forms:error-min-code'),
})
...
export const OTPLoginForm: React.FC<OTPProps> = ({ onLoginSuccess }) => {
    ...
    const { mutate: otpLogin, isLoading: otpLoginLoading } = useOtpLoginMutation()
