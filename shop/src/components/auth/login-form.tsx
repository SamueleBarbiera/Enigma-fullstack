...
const loginFormSchema = yup.object().shape({
    email: yup.string().email('forms:email-error').required('forms:email-required'),
    ...
    return (
        <div className='overflow-hidden bg-white mx-auto rounded-lg w-full sm:w-96 md:w-450px border border-gray-300 py-5 px-5 sm:px-8'>
            <div className='text-center mb-6 pt-2.5'>
                ...
                <p className='text-sm md:text-base text-body mt-2 mb-8 sm:mb-10'>{t('common:login-helper')}</p>
            </div>

            {errorMessage && <Alert message={errorMessage} variant={'error'} className='my-3' />}

            <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col justify-center' noValidate>
                <div className='flex flex-col space-y-3.5'>
                    <Input labelKey='forms:label-email-star' type='email' variant='solid' {...register('email')} errorKey={errors.email?.message} />
                    <PasswordInput labelKey='forms:label-password-star' errorKey={errors.password?.message} {...register('password')} />
                    <div className='flex items-center justify-center'>
                        <div className='flex items-center flex-shrink-0'>
                            <label className='switch relative inline-block w-10 cursor-pointer'>
                                <input id='remember' type='checkbox' className='opacity-0 w-0 h-0' {...register('remember_me')} />
                                <span className='bg-gray-500 absolute inset-0 transition-all duration-300 ease-in slider round' />
                            </label>
                            <label htmlFor='remember' className='flex-shrink-0 text-sm text-heading ltr:pl-3 rtl:pr-3 cursor-pointer'>
                    ...
                    <div className='relative'>
                        <Button type='submit' loading={loading} disabled={loading} className='h-11 md:h-12 w-full mt-1.5'>
                            {t('common:text-login')}
                        </Button>
                    </div>
                </div>
            ...
            <div className='grid grid-cols-1 gap-4 mt-2'>
                <Button loading={false} disabled={false} className='h-11 md:h-12 w-full mt-2.5 bg-google hover:bg-googleHover' onClick={() => signIn('google')}>
            ...
            <div className='text-sm sm:text-base text-body text-center mt-5 mb-1'>
                ...
                <button type='button' className='text-sm sm:text-base text-heading underline font-bold hover:no-underline focus:no-underline focus:outline-none' onClick={handleSignUp}>
