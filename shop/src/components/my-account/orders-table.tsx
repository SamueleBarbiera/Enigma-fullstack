...
const OrdersTable: React.FC<Props> = ({ orders, onPagination }: Props) => {
    ...
    return (
        <>
            <h2 className="text-lg md:text-xl xl:text-2xl font-bold text-heading mb-6 xl:mb-8">{t('text-orders')}</h2>
                ...
                {width >= 1025 ? (
                    <table>
                        ...
                        <tbody className="text-sm lg:text-base">
                            {data && data?.map((order: Order) => <OrderSingleList key={order.id} order={order} />)}
                        </tbody>
                    </table>
                ) : (
                    <div className="w-full space-y-4">
                        {data && data?.map((order: Order) => <OrderSingleTable key={order.id} order={order} />)}
                    </div>
                )}
            </div>
            {/* Pagination */}
