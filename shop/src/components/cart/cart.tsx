    ...
    return (
        <div className="flex flex-col w-full h-full justify-between">
            <div className="w-full flex justify-between items-center relative ltr:pl-5 rtl:pr-5 ltr:md:pl-7 rtl:md:pr-7 py-0.5 border-b border-gray-100">
                <h2 className="font-bold text-xl md:text-2xl m-0 text-heading">{t('text-shopping-cart')}</h2>
                <button
            ...
            {!isEmpty ? (
                ...
                >
                    <EmptyCart />
                    <h3 className="text-lg text-heading font-bold pt-8">{t('text-empty-cart')}</h3>
                </motion.div>
            )}

            <div className="flex flex-col px-5 md:px-7 pt-2 pb-5 md:pb-7" onClick={closeCart}>
                <Link
