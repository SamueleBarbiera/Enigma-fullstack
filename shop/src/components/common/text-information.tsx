...
const TextInformation: React.FC<Props> = ({ item, className }) => {
    ...
    return (
        ...
        >
            ...
            <div className="-mb-1">
                <h3 className="text-heading text-base md:text-lg font-semibold mb-1.5 md:mb-2">{t(`${item.title}`)}</h3>
                <p className="text-body text-xs md:text-sm leading-6 md:leading-7">{t(`${item.description}`)}</p>
            </div>
        </div>
    )
}
