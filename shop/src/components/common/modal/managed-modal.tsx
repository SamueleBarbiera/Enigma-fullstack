...
const ForgetPasswordForm = dynamic(() => import('@components/auth/forget-password/forget-password'))
...
const ManagedModal: React.FC = () => {
    ...
    return (
        <Modal open={displayModal} onClose={closeModal} variant={modalVariant}>
            ...
            {modalView === 'ADD_OR_UPDATE_CHECKOUT_CONTACT' && <AddOrUpdateCheckoutContact data={modalData} />}
            {modalView === 'ADD_OR_UPDATE_PROFILE_CONTACT' && <ProfileAddOrUpdateContact data={modalData} />}
        </Modal>
    )
}
