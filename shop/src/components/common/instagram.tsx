...
const Instagram: React.FC<Props> = ({ className = '' }) => {
    const { data: instagramFeed, isLoading: loading } = useInstagram({
        limit: 6,
    })
    ...
    return (
        <div className={cn('grid grid-cols-3 md:grid-cols-6 gap-0.5 sm:gap-1 overflow-hidden rounded-md', className)}>
            {!loading &&
