...
import { couponAtom, discountAtom, verifiedResponseAtom } from '@store/checkout'
            ...
            <div className="flex flex-col">
                ...
                {!isEmptyCart ? (
                    <div className="px-6 py-2.5">
                        {items?.map((item) => {
                            const notAvailable = verifiedResponse?.unavailable_products?.find((d: any) => d === item.id)
                            return <ItemCard item={item} key={item.id} notAvailable={!!notAvailable} />
                        })}
                    </div>
                ) : (
                    <EmptyCartIcon />
