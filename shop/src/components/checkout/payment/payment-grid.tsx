...
const AVAILABLE_PAYMENT_METHODS_MAP: Record<PaymentMethodName, PaymentMethodInformation> = {
    STRIPE: {
        name: 'Stripe',
        value: 'STRIPE',
        ...
        <div className={className}>
            ...
            <RadioGroup value={gateway} onChange={setGateway}>
                ...
                <div className="grid gap-4 grid-cols-2 md:grid-cols-3 mb-6">
                    {Object.values(AVAILABLE_PAYMENT_METHODS_MAP).map(({ name, icon, value }) => (
                        <RadioGroup.Option value={value} key={value}>
                            {({ checked }) => (
                                <div
                                    className={cn(
                                        'w-full h-full py-3 flex items-center justify-center border text-center rounded cursor-pointer relative',
                                        checked ? 'bg-white border-gray-600 shadow-600' : 'bg-white border-gray-100'
                                    )}
                                >
                                    {icon ? (
                                        <>
                                        ...
                                        <span className="text-xs text-heading font-semibold">{name}</span>
                                    )}
                                </div>
                            )}
                        </RadioGroup.Option>
