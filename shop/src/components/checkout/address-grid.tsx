...
export const AddressGrid: React.FC<AddressesProps> = ({ addresses, label, atom, className, userId, count, type }) => {
                ...
                <RadioGroup value={selectedAddress} onChange={setAddress}>
                    ...
                    <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-2 xl:grid-cols-3">
                        {addresses?.map((address) => (
                            <RadioGroup.Option value={address} key={address.id} className="focus-visible:outline-none">
                                {({ checked }) => <AddressCard checked={checked} address={address} userId={userId} />}
                            </RadioGroup.Option>
                        ))}
                    </div>
                </RadioGroup>
