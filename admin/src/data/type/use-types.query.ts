...
const fetchTypes = async ({ queryKey }: QueryParamsType): Promise<{ types: TypesPaginator }> => {
    ...
    const { page, text, limit = 15, orderBy = 'updated_at', sortedBy = 'DESC' } = params as TypesQueryOptionsType
...
const useTypesQuery = (options: TypesQueryOptionsType) => {
    return useQuery<{ types: TypesPaginator }, Error>([API_ENDPOINTS.TYPES, options], fetchTypes, {
