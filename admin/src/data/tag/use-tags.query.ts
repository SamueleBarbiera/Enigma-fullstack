...
const fetchTags = async ({ queryKey }: QueryParamsType): Promise<{ tags: TagPaginator }> => {
    const [_key, params] = queryKey

    const { page, text, type, limit = 15, orderBy = 'updated_at', sortedBy = 'DESC' } = params as TagsQueryOptionsType
...
const useTagsQuery = (options: TagsQueryOptionsType) => {
    return useQuery<{ tags: TagPaginator }, Error>([API_ENDPOINTS.TAGS, options], fetchTags, {
