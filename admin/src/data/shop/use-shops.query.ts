...
const fetchShops = async ({ queryKey }: QueryParamsType): Promise<{ shops: ShopPaginator }> => {
    const [_key, params] = queryKey

    const { page, text, limit = 15, orderBy = 'updated_at', sortedBy = 'DESC' } = params as ShopsQueryOptionsType
...
const useShopsQuery = (options: ShopsQueryOptionsType) => {
    return useQuery<{ shops: ShopPaginator }, Error>([API_ENDPOINTS.SHOPS, options], fetchShops, {
