import { QueryParamsType, CategoriesQueryOptionsType } from '@ts-types/custom.types'
...
const fetchCategories = async ({ queryKey }: QueryParamsType): Promise<{ categories: CategoryPaginator }> => {
...
const useCategoriesQuery = (options: CategoriesQueryOptionsType) => {
    return useQuery<{ categories: CategoryPaginator }, Error>([API_ENDPOINTS.CATEGORIES, options], fetchCategories, {
