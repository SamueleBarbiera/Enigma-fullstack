...
export default function CreateOrUpdateOrderStatusForm({ initialValues }: any) {
    ...
    const { mutate: createOrderStatus, isLoading: creating } = useCreateOrderStatusMutation()
    const { mutate: updateOrderStatus, isLoading: updating } = useUpdateOrderStatusMutation()
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
            <div className="my-5 flex flex-wrap sm:my-8">
                <Description
                    ...
                    details={`${initialValues ? t('form:button-label-update') : t('form:button-label-add')} ${t(
                        'form:order-status-description-helper-text'
                    )}`}
