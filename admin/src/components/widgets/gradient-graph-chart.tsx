...
const GradientGraphChart = ({ series, topRowTitle, bottomRowData, labels, colors }: any) => {
    const options = {
        options: {
            chart: {
                toolbar: {
    ...
    return (
        <div className="h-full w-full rounded bg-light shadow-sm">
            ...
            <div className="w-full">
                <Charts options={options.options} series={options.series} width="100%" type="area" />
            </div>
                ...
                {bottomRowData &&
                    bottomRowData.map((item: any, index: number) => (
                        <div className="flex flex-col" key={index}>
                            ...
                            <span className="mt-1 mb-2 text-lg font-semibold text-heading">{item.valueText}</span>
