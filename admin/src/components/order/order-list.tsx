...
const OrderList = ({ orders, onPagination, onSort, onOrder }: IProps) => {
    ...
    const columns: readonly (ColumnGroupType<Order> | ColumnType<Order>)[] = [
        ...
        },
        {
            title: (
                <TitleWithSort
                    ...
                    ascending={sortingObj.sort === SortOrder.Asc && sortingObj.column === 'status'}
        ...
        },
        {
            ...
            render: (_id: string, order: Order) => (
                <div className="block">
                    ...
                    >
                        {({ loading }) => (loading ? t('common:text-loading') : t('common:text-download'))}
                    </PDFDownloadLink>
                </div>
            ),
        },
        {
            ...
            align: 'center',
            width: 100,
            render: (id: string) => <ActionButtons id={id} detailsUrl={`${router.asPath}/${id}`} />,
        },
    ]
